<?php
	...
	if(isset($_POST['search']) && strlen($_POST['search']) !== 0)
	{
		...
		$_SESSION['condition'] = " title LIKE '%".calendar::db_escape($_POST['search'])."%' OR description LIKE '%".calendar::db_escape($_POST['search'])."%'";
	...
	if(isset($_POST['filter']) && !strlen($_POST['filter']) !== 0)
	{
		...
		if($filter == 'all-fields')
		{
			...
			$_SESSION['condition'] = "category = '".calendar::db_escape($filter)."'";	
		}
