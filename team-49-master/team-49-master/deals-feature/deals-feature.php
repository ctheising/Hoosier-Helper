...
function createCommentRow($data) {
    ...
    $response = '
            <div class="comment">
                ...
                <div class="specialReply"><a href="javascript:void(0)" data-commentID="'.$data['discID'].'" onclick="reply(this)">REPLY</a></div>
                <div class="replies">';

    $sql = $conn->query("SELECT specialReply.specreplyID, fname, contents, DATE_FORMAT(specialReply.date, '%Y-%m-%d') AS createdOn FROM specialReply INNER JOIN user ON specialReply.uID = user.uID WHERE specialReply.discID = '".$data['discID']."' ORDER BY specialReply.specreplyID DESC LIMIT 1");
...
if (isset($_POST['getAllComments'])){
    ...
    $response = "";
    $sql = $conn->query("SELECT specialDisc.discID, fname, contents, DATE_FORMAT(specialDisc.date, '%Y-%m-%d') AS createdOn FROM specialDisc INNER JOIN user ON specialDisc.uID = user.uID ORDER BY specialDisc.discID DESC LIMIT $start, 20");
...
if (isset($_POST['addComment'])){
    ...
    if($isReply != 'false') {
        $conn->query("INSERT INTO specialReply (contents, discID, uID, date) VALUES ('$comment', '$commentID', '" . $_SESSION['uID'] . "', NOW())");
        $sql = $conn->query("SELECT specialReply.discID, fname, contents, DATE_FORMAT(specialReply.date, '%Y-%m-%d') AS createdOn FROM specialReply INNER JOIN user ON specialReply.uID = user.uID ORDER BY specialReply.discID DESC LIMIT 1");

    }
    else {
        $conn->query("INSERT INTO specialDisc (uID, contents, date) VALUES('" . $_SESSION['uID'] . "','$comment', NOW())");
        $sql = $conn->query("SELECT specialDisc.discID, fname, contents, DATE_FORMAT(specialDisc.date, '%Y-%m-%d') AS createdOn FROM specialDisc INNER JOIN user ON specialDisc.uID = user.uID ORDER BY specialDisc.discID DESC LIMIT 1");
    }
...
$sqlNumComments = $conn->query("SELECT discID FROM specialDisc");
...
<head>
	...
	<nav class="rvt-menu" aria-label="Section navigation">
	  <ul class="rvt-menu__list">
		...
		<li class="rvt-menu__item">
		  <a href="../deals-feature/deals-feature.php" aria-current="page">Specials</a>
		</li>
		<li class="rvt-menu__item">
	...
	<div class="rvt-box">
	  <div class="rvt-box__header">
		<h1 class="rvt-ts-md">Local Food Specials</h1>
	  </div>
	  <div class="rvt-box__body">
	  	<center>
			<p class="rvt-m-all-remove">
		
			<img src="pizza_x_logo.png" width="100px" height="100px">
			<p>Get 2 to 10 Large 1-Topping Pizzas for $9.99 each. Click <a href="https://pizzax-campus.foodtecsolutions.com/ordering/menu/Specials">here</a>
			to order now!</p>
		
		
			<img src="papa_johns_logo.svg" width="100px" height="100px">
			<br>
			Get the New Double Cheeseburger Papadia. <a href="https://www.papajohns.com/order/menu?category=Papadias">Order Now</a>
		
			<br>
			<img src="brothers_logo.jpg" width="120px" height="50px">
			<br>
			$2 Double Wells and more Monday through Sunday. Click <a href="https://www.brothersbar.com/bloomington">here</a> to view more specials.
		
			<br>
			<img src="tacobell_logo.png" width="65px" height="100px">
			<br>
			Limited Time! Nacho Fries Box for $5. <a href="https://www.tacobell.com/food/deals-and-combos/nacho-fries-box?store=028901">Order Now</a>
		
		
			</p>
		</center>
	  </div>
	  <div class="rvt-box__footer rvt-text-center">
		<strong class="rvt-text-regular">Know of more specials? Leave a comment below for others to explore!</strong>
	  </div>
	</div>
		...
		<div class="container">
    		<div class="row">
        		<div class="col-md-12">
           			<textarea class="form-control" id="mainComment" placeholder="Add Specials" cols="30" rows="2"></textarea><br>
    	...
    	$(document).ready(function (){
        	$("#addComment, #addReply").on('click', function () {
            	...
            	if (comment.length > 5) {
                	$.ajax({
                   		url: 'https://cgi.luddy.indiana.edu/~team49/team-49/deals-feature/deals-feature.php',
                    	method: 'POST',
                    	dataType: 'text',
                    	data: {
                        	addComment: 1,
