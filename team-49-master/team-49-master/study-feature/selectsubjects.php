  ...
  <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.6/js/bootstrap.min.js"></script>
	...
	<div class="rvt-box__header">
		<h1 class="rvt-ts-md">Study Groups</h1>
			...
			<?php
			$con = mysqli_connect("db.luddy.indiana.edu", "i494f20_team49", "my+sql=i494f20_team49", "i494f20_team49");
			// Check connection
			if (mysqli_connect_errno())
			{echo "Failed to connect to MySQL: " . mysqli_connect_error() . "<br>"; }

			// Grab POST Data
			$var_adv = mysqli_real_escape_string($con, $_POST['subject']);
			$sql="	SELECT name, subject, location, day, time
			FROM studyGroups 
			WHERE groupID='$var_adv';";

			$result = mysqli_query($con, $sql);

			if ($result->num_rows > 0) {
				echo "<table border='1'><tr><th>Name</th><th>Subject</th><th>Location</th><th>Day of Week</th><th>Time</th></tr>";
					while($row = $result->fetch_assoc()) {
					echo 
					"<tr><td>".$row["name"]."</td><td>".$row["subject"]."</td><td>".$row["location"]."</td><td>".$row["day"]."</td><td>".$row["time"]."</td></tr>";
				}
				echo "</table>";
			} else {
				echo "0 results <br>";
		...
		<p>Click Here to return to previous page-  <a href="../study-feature/study.php" class="rvt-link-bold">Study Groups</a>.</p>
